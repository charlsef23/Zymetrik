...
enum TemplateEngineLite {
    ...
    static func buildPreview(
        ...
        var fuerza = catalog.filter {
            $0.tipo.lowercased().contains("gimnasio") || $0.categoria.lowercased().contains("fuerza")
        }
        var cardio = catalog.filter {
            $0.tipo.lowercased().contains("cardio") || $0.categoria.lowercased().contains("cardio")
        }

        if fuerza.isEmpty {
            fuerza = catalog.filter {
                nameMatch($0.nombre, ["press","squat","peso muerto","remo","dominada","hombro","banco","curl","tríceps"])
            }
        }
        if cardio.isEmpty {
            cardio = catalog.filter {
                nameMatch($0.nombre, ["carrera","run","bike","bici","remo","elíptica","caminata","saltar","burpee"])
            }
        }

...
// MARK: - Enums base (si ya los tienes en otro archivo, elimina esta sección)
...
enum FocoPlan: String, CaseIterable, Identifiable {
    case fuerza = "Fuerza"
    case cardio = "Cardio"
...
struct PlantillasPROView: View {
    ...
    /// Catálogo completo (los mismos ejercicios que usa ListaEjerciciosView)
    ...
    var plantillas: [WeeklyTemplateLite] {
        [
                    ...
                    VStack(spacing: 10) {
                        ...
                        Button {
                            ...
                            HStack {
                                ...
                                Text("Aplicar rutina al calendario")
                                    .bold()
                            }
                        ...
                        .disabled(previewSemana.isEmpty || (subs.isPro == false))
            ...
            .toolbar {
                if let name = routine.activePlanName {
                    ToolbarItem(placement: .topBarTrailing) {
                        RoutinePillButton(title: name) { /* Abrir gestión de rutina si quieres */ }
                    }
                }
            }
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            Text("Elige tu plan de entrenamiento")
                .font(.headline)
            Spacer()
    ...
    private var filtros: some View {
        VStack(spacing: 12) {
            Picker("Nivel", selection: $nivel) {
                ForEach(NivelEntrenamiento.allCases) { n in
            ...
            Picker("Foco", selection: $foco) {
                ForEach(FocoPlan.allCases) { f in
    ...
    private func onTap(template: WeeklyTemplateLite) {
        ...
        // 🔸 Si quieres usar el motor Pro (PPL/híbrido real), cambia por:
        /*
        let spec = PlanSpec(
            modo: template.foco == .fuerza ? .ppl : (template.foco == .cardio ? .cardio : .hibrido),
            diasPorSemana: template.diasSemana,
            nivel: template.nivel,
            includeCardio: (template.foco != .cardio),
            weekdays: weekdaysSeleccionados
        )
        previewSemana = TemplateEnginePro.buildPreview(catalog: ejerciciosCatalogo, spec: spec)
        */
    }
    ...
    private func aplicarRutina() async {
        ...
        // Marca rutina activa en el tracker
...
// MARK: - Selector de días (1=Dom..7=Sáb; orden L..D)
